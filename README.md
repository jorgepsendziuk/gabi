...
O `db:seed` já cria listagem e mapa para `public.familias`. Recursos gerados:

- `public_familias_list` — listagem com busca, paginação e export CSV  
- `public_familias_map` — mapa MapLibre com 25 pontos PostGIS  

Para gerar outras tabelas:

```bash
pnpm gabi generate --schema public --table familias --template list
pnpm gabi generate --schema public --table familias --template map
```

Ou use a UI em **Banco de dados** → botões **Lista** / **Mapa**.

### 5. Cadastrar outro banco (opcional)

No admin: **Conexões** → preencher host, database, usuário, senha → **Testar** → **Salvar**.

Depois em **Banco de dados**, escolha a conexão no seletor e gere listas/mapas.
...
2. **Banco de dados** → gere lista/mapa; tabelas ODK detectadas usam overlay automaticamente.
...
1. Login no admin  
2. **Banco de dados** — introspectar tabelas  
3. Gerar **Lista** e/ou **Mapa** para `public.familias`  
4. Abrir páginas geradas no painel  
5. Exportar CSV, ver mapa com pontos PostGIS  
